...
```bash
cd StopMotionApp
open StopMotion.xcodeproj   # Xcode app project – pick the "StopMotion" scheme and press ⌘R
# or
open Package.swift          # the same code as a Swift package
# or
...
`StopMotion.xcodeproj` is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen) from
`StopMotionApp/project.yml`. After adding or removing Swift files, run `xcodegen generate` in
`StopMotionApp/` to regenerate it.
...
1. **.NET 10.0 SDK** - Download from [Microsoft .NET](https://dotnet.microsoft.com/download)
...
./bin/Release/net10.0/ImgConcat "/path/to/your/images/folder"
```
